...
export default function StoreRequestByRack({ onBack, currentUser, activeBranch }) {
    ...
    const addToCart = (product, qty = 1) => {
        ...
        if (existingItem) {
            ...
            setCart(prev => prev.map(item =>
                item.barcode === product.barcode_no
            ...
            setCart(prev => [...prev, {
                ...
                product_name_la: product.item_name,
                qty,
                rack_location: product.rack_location,
                stock_at_request: product.qty,
                branch_id: userBranch
            }]);
            new Audio(sfxOK).play().catch(() => { });
            toast.success(`ເພີ່ມ "${product.item_name?.slice(0, 20)}" ເຂົ້າກະຕ່າແລ້ວ ✓`);
        }
    };
    ...
    const rackProductsPanelJSX = (
        <div className="space-y-5">
            ...
            <div className="bg-white dark:bg-slate-800/90 backdrop-blur-md rounded-3xl p-4 md:p-6 border border-slate-200 dark:border-slate-700/80 shadow-xl space-y-4">
                <div className="flex items-center justify-between">
                    <label className="text-xs md:text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-wider flex items-center gap-2">
                        <ScanLine size={18} className="text-orange-400" />
                        <span>ສະແກນ ຫຼື ພິມ ໂລເຄຊັ້ນ / ແຣັກ (Rack Location)</span>
                    </label>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium hidden sm:inline">ຕົວຢ່າງ: G01-L1-1, MFA01-1</span>
                </div>
                ...
                <form onSubmit={handleRackSubmit} className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-slate-100 dark:bg-slate-950/90 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-bold placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 text-sm md:text-base transition-all"
                        />
                        {rackLocationInput && (
                            <button
                                type="button"
                                onClick={() => setRackLocationInput('')}
                                className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-700 dark:hover:text-white"
                            >
                                <X size={16} />
                            </button>
                        )}
                    ...
                    <button
                        ...
                        className="px-4 py-3.5 rounded-2xl bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white border border-slate-300 dark:border-slate-600 flex items-center justify-center transition-all cursor-pointer shrink-0"
                            ...
                            {recentRacks.map(r => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer border ${activeRack === r
                                            ...
                                            : 'bg-slate-100 dark:bg-slate-900/60 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-300 dark:border-slate-700'
                                        }`}
                                >
                                    📍 {r}
                                </button>
            ...
            {activeRack && (
                <div className="bg-white dark:bg-slate-800/90 backdrop-blur-md rounded-3xl p-4 md:p-6 border border-slate-200 dark:border-slate-700/80 shadow-xl space-y-4">
                    {/* Header & Filter Bar */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-700/70 pb-4">
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-base md:text-lg font-black text-slate-900 dark:text-white">ລາຍການສິນຄ້າໃນ Rack ນີ້</h2>
                            </div>
                        ...
                        {/* Status Filters */}
                        <div className="flex items-center bg-slate-100 dark:bg-slate-950/80 p-1 rounded-2xl border border-slate-200 dark:border-slate-700 gap-1 shrink-0 overflow-x-auto">
                            <button
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-950/70 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-medium placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-orange-500"
                        />
                    </div>
                    ...
                    {isLoadingProducts ? (
                        ...
                        <div className="py-12 text-center bg-slate-50 dark:bg-slate-950/40 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 p-6 space-y-2">
                            <Package size={36} className="text-slate-600 mx-auto" />
                            <p className="text-sm font-bold text-slate-600 dark:text-slate-300">ບໍ່ພົບລາຍການສິນຄ້າທີ່ກົງກັບຟິວເຕີ</p>
                            <p className="text-xs text-slate-400 dark:text-slate-500">ລອງປ່ຽນຟິວເຕີ ຫຼື ຄົ້ນຫາແຣັກອື່ນ</p>
                        </div>
                    ) : (
                        <div className="space-y-4 max-h-[620px] overflow-y-auto pr-1">
                            {filteredProducts.map((product, idx) => {
                                ...
                                const totalCalculated = (product.qty || 0) + (product.warehouse_qty || 0);

                                return (
                                    <div
                                        key={product.barcode_no || idx}
                                        className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-4 md:p-5 shadow-lg dark:shadow-2xl space-y-3.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                                    >
                                        {/* Card Top: Barcode Pill Badge & Status Tag */}
                                        <div className="flex items-center justify-between gap-2">
                                            <div className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-mono font-black text-xs md:text-sm px-3.5 py-1 rounded-full shadow-md tracking-wider">
                                                {product.barcode_no || 'N/A'}
                                            </div>

                                            {/* Status Badge Tag */}
                                            {isOutOfStock ? (
                                                <span className="px-2.5 py-1 rounded-full bg-red-950/80 border border-red-800/80 text-red-400 font-black text-[10px] uppercase tracking-wider">
                                                    OUT OF STOCK
                                                </span>
                                            ) : isLowStock ? (
                                                <span className="px-2.5 py-1 rounded-full bg-amber-950/80 border border-amber-800/80 text-amber-400 font-black text-[10px] uppercase tracking-wider">
                                                    LOW STOCK
                                                </span>
                                            ) : (
                                                <span className="px-2.5 py-1 rounded-full bg-emerald-950/80 border border-emerald-800/80 text-emerald-400 font-black text-[10px] uppercase tracking-wider">
                                                    IN STOCK
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <div className="space-y-1.5">
                                            <h3 className="text-sm md:text-base font-bold text-slate-900 dark:text-white leading-snug line-clamp-2">
                                                {product.item_name}
                                            </h3>

                                            <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                                                {product.category_1_actual && (
                                                    <span className="px-2 py-0.5 rounded-md bg-emerald-950/60 border border-emerald-800/50 text-emerald-400 text-[10px] font-black uppercase">
                                                        {product.category_1_actual}
                                                    </span>
                                                )}
                                                {product.product_tag && (
                                                    <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold uppercase">
                                                        {product.product_tag}
                                                    </span>
                                                )}
                                                {/* 🏷️ Display Max Qty (ຄວາມຈຸສູງສຸດ) */}
                                                <span className="px-2 py-0.5 rounded-md bg-amber-500/10 dark:bg-amber-500/20 border border-amber-500/30 text-amber-600 dark:text-amber-300 text-[10px] font-black">
                                                    🎯 Max: {product.max_qty > 0 ? product.max_qty : 'ບໍ່ໄດ້ກຳນົດ'}
                                                </span>
                                            </div>
                                        </div>

                                        {/* 4-GRID STOCK STATS (Strictly matched with image layout) */}
                                        <div className="grid grid-cols-4 gap-2 pt-1">
                                            {/* 1. TOTAL */}
                                            <div className="bg-amber-50 dark:bg-[#161224] border border-amber-300/50 dark:border-amber-500/20 rounded-2xl p-2.5 text-center flex flex-col justify-center">
                                                <span className="text-amber-400 font-black text-lg md:text-xl leading-none">
                                                    {totalCalculated}
                                                </span>
                                                <span className="text-[9px] font-black text-amber-500/80 uppercase tracking-wider mt-1">
                                                    TOTAL
                                                </span>
                                            </div>

                                            {/* 2. หน้าร้านรวม */}
                                            <div className="bg-emerald-50 dark:bg-[#0f1d1d] border border-emerald-300/50 dark:border-emerald-500/20 rounded-2xl p-2.5 text-center flex flex-col justify-center">
                                                <span className="text-emerald-400 font-black text-lg md:text-xl leading-none">
                                                    {product.qty}
                                                </span>
                                                <span className="text-[9px] font-black text-emerald-700 dark:text-emerald-500/80 uppercase tracking-wider mt-1">
                                                    ໜ້າຮ້ານ
                                                </span>
                                            </div>

                                            {/* 3. สาง (Warehouse) */}
                                            <div className="bg-sky-50 dark:bg-[#101b2b] border border-sky-300/50 dark:border-sky-500/20 rounded-2xl p-2.5 text-center flex flex-col justify-center">
                                                <span className="text-sky-400 font-black text-lg md:text-xl leading-none">
                                                    {product.warehouse_qty || 0}
                                                </span>
                                                <span className="text-[9px] font-black text-sky-700 dark:text-sky-500/80 uppercase tracking-wider mt-1">
                                                    ສາງ
                                                </span>
                                            </div>

                                            {/* 4. DC (Distribution Center) */}
                                            <div className="bg-purple-50 dark:bg-[#19132b] border border-purple-300/50 dark:border-purple-500/20 rounded-2xl p-2.5 text-center flex flex-col justify-center">
                                                <span className="text-purple-400 font-black text-lg md:text-xl leading-none">
                                                    0
                                                </span>
                                                <span className="text-[9px] font-black text-purple-600 dark:text-purple-400/80 uppercase tracking-wider mt-1">
                                                    DC
                                                </span>
                                            </div>
                                        </div>

                                        {/* Location Footer Bar & Request Action Button */}
                                        <div className="pt-2 flex items-center justify-between gap-3">
                                            {/* Location Pill */}
                                            <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#121826] border border-slate-300 dark:border-slate-800 px-3 py-2 rounded-2xl text-xs font-mono font-bold text-slate-700 dark:text-slate-200">
                                                <MapPin size={14} className="text-emerald-400 shrink-0" />
                                                <span className="truncate">{product.rack_location || activeRack}</span>
                                            </div>

                                            {/* Action Button: Inline One-Click Stepper (No pop-up needed) */}
                                            {inCartItem ? (
                                                <div className="flex items-center gap-1 bg-emerald-50 dark:bg-[#121c27] border border-emerald-400/50 dark:border-emerald-500/50 p-1 rounded-2xl shadow-lg shadow-emerald-500/10">
                                                    <button
                                                        onClick={() => updateCartQty(product.barcode_no, -1)}
                                                        className="w-8 h-8 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 active:scale-90 text-slate-900 dark:text-white font-black flex items-center justify-center transition-all cursor-pointer"
                                                        title="ຫຼຸດຈຳນວນ"
                                                    >
                                                        <Minus size={14} />
                                                    </button>
                                                    <input
                                                        type="number"
                                                        min="1"
                                                        value={inCartItem.qty === '' ? '' : inCartItem.qty}
                                                        onChange={(e) => {
                                                            // Allow empty string while typing so user can clear and retype
                                                            const raw = e.target.value;
                                                            setCart(prev => prev.map(c =>
                                                                c.barcode === product.barcode_no
                                                                    ? { ...c, qty: raw === '' ? '' : Math.max(1, Number(raw) || 1) }
                                                                    : c
                                                            ));
                                                        }}
                                                        onBlur={(e) => {
                                                            ...
                                                            if (!val || val < 1) {
                                                                setCart(prev => prev.map(c =>
                                                                    c.barcode === product.barcode_no ? { ...c, qty: 1 } : c
                                                                ));
                                                            }
                                                        }}
                                                        className="w-11 text-center font-black text-sm text-emerald-400 bg-transparent outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                                    />
                                                    <button
                                                        onClick={() => updateCartQty(product.barcode_no, 1)}
                                                        className="w-8 h-8 rounded-xl bg-emerald-500 hover:bg-emerald-600 active:scale-90 text-slate-950 font-black flex items-center justify-center transition-all cursor-pointer shadow-md shadow-emerald-500/20"
                                                        title="ເພີ່ມຈຳນວນ"
                                                    >
                                                        <Plus size={14} />
                                                    </button>
                                                    <button
                                                        onClick={() => removeFromCart(product.barcode_no)}
                                                ...
                                                <button
                                                    ...
                                                    className="px-4 py-2 rounded-2xl bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-slate-950 font-black text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-lg shadow-emerald-500/20"
                                                >
                                                    <Plus size={15} />
    ...
    // ─── CART PANEL JSX ───────────────────────────────────────────────
    const cartPanelJSX = (
        <div className="bg-white dark:bg-slate-800/90 backdrop-blur-md rounded-3xl p-4 md:p-6 border border-slate-200 dark:border-slate-700/80 shadow-xl space-y-4 h-full flex flex-col justify-between">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-3">
                <div className="flex items-center gap-2">
                    <ShoppingBag size={20} className="text-orange-400" />
                    <h2 className="text-base md:text-lg font-black text-slate-900 dark:text-white">ກະຕ່າຄຳຂໍ (Request Cart)</h2>
                </div>
            ...
            {cart.length === 0 ? (
                <div className="my-auto py-12 text-center space-y-3 bg-slate-50 dark:bg-slate-950/40 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700/70 p-4">
                ...
                <div className="flex-1 flex flex-col justify-between space-y-4 min-h-0">
                    <div className="space-y-2.5 overflow-y-auto pr-1 flex-1">
                        {cart.map(item => (
                            <div key={item.barcode} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-700 space-y-2">
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <span className="font-mono text-[10px] text-slate-400 dark:text-slate-400">{item.barcode}</span>
                                    <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-xl p-1 border border-slate-200 dark:border-slate-700">
                                        <button
                                            ...
                                            className="w-6 h-6 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 flex items-center justify-center text-slate-700 dark:text-white font-bold"
                                        >
                                            <Minus size={12} />
                                        </button>
                    ...
                    <div className="pt-3 border-t border-slate-200 dark:border-slate-700 space-y-3 shrink-0">
                        <div className="flex justify-between items-center text-sm font-bold">
                            <span className="text-slate-500 dark:text-slate-400">ຈຳນວນລວມທັງໝົດ:</span>
    ...
    return (
        <div
            className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 pb-24 relative overflow-x-hidden"
        ...
        >
            ...
            {/* Header Navbar */}
            <header className="sticky top-0 z-30 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 py-3.5 shadow-lg">
                <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        {onBack && (
                            <button
                                onClick={onBack}
                                className="p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-700/60 transition-all active:scale-95 cursor-pointer"
                                title="ກັບຄືນ"
                            >
                                <ArrowLeft size={20} />
                            </button>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1.5">
                                ...
                                <span>ສາຂາ: <strong className="text-slate-900 dark:text-white">{userBranch}</strong></span>
                                <span className="opacity-40">•</span>
                                <span>ຜູ້ຂໍ: <strong className="text-slate-600 dark:text-slate-300">{currentUser?.name || 'Staff'}</strong></span>
                            </p>
                        </div>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4">
                ...
                <div className="md:hidden flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-slate-700 mb-4">
                    <button
