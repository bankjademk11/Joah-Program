...
const StoreRequest = ({ onBack, currentUser }) => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (data) {
                ...
                setQty(1);
                if (data.qty > 0) {
                    toast.success(`ພົບສິນຄ້າ! ມີໃນสาง: ${data.qty} ຫນ່ວຍ`);
                } else {
                    toast.warning('ພົບสິນຄ້າ, ແຕ່สາງໝົດແລ້ວ');
                }
            } else {
                setProduct(null);
                toast.error('ບໍ່ພົບສินຄ້ານີ້');
            }
        } catch (err) {
    ...
    const handleSubmitCart = async () => {
        ...
        try {
            ...
            if (error) throw error;
            toast.success(`✅ ສົ່ງຄຳຂໍ ${cart.length} รายการสำเร็จ!`);
            setCart([]);
            fetchRecentRequests();
        } catch (err) {
        ...
        <div className="w-full max-w-6xl mx-auto p-4 md:p-6 animate-fade-in-up flex flex-col md:flex-row gap-6 h-[calc(100vh-100px)]">
            ...
            <div className="flex-1 flex flex-col gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div><h1 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Store Request</h1><p className="text-sm font-bold text-slate-400 uppercase tracking-widest">ລະບົບຂໍສິນຄ້າໜ້າຮ້ານ</p></div>
                </div>
                <div className="glass-card p-8 rounded-[2rem] flex flex-col gap-6 shadow-xl relative overflow-hidden">
                    ...
                    <form onSubmit={handleSearch} className="relative z-10">
                        ...
                        <div className="flex gap-2">
                            <input ref={barcodeInputRef} type="text" value={barcode} onChange={(e) => setBarcode(e.target.value)} placeholder="ຍິງບາໂຄດບ່ອນນີ້..." className="w-full text-2xl font-bold bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-200 dark:border-slate-700 rounded-2xl px-6 py-4 placeholder:text-slate-300" autoFocus />
                    ...
                    {product && (
                        <div className="animate-scale-in bg-white dark:bg-slate-800/50 rounded-3xl p-6 border-2 border-blue-100 dark:border-blue-500/30 relative z-10">
                            ...
                            <div className={`flex items-center gap-3 mb-6 p-4 rounded-2xl ${product.available_qty > 0 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200' : 'bg-rose-50 dark:bg-rose-500/10 border-rose-200 border'}`}>
                                ...
                                <div className="flex-1"><p className="text-xs font-black uppercase tracking-wider">{product.available_qty > 0 ? 'ມີໃນສາງ' : 'สາງໝົດ'}</p><p className="text-lg font-black">{product.available_qty} ຫນ່ວย</p></div>
                            </div>
                            ...
                            <button onClick={handleAddToCart} className="w-full py-4 bg-gradient-to-r from-joah-orange to-orange-600 hover:from-orange-500 hover:to-orange-500 text-white rounded-2xl font-black text-lg tracking-wide shadow-xl shadow-orange-500/30 transition-all flex items-center justify-center gap-3"><Plus size={24} /><span>ເພີ່ມລົງລາຍການ</span></button>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="md:w-96 flex flex-col gap-6 h-full overflow-hidden">
                <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-6 shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col flex-1 min-h-[300px]">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-black text-slate-800 dark:text-white">ລາຍການທີ່ເລືອກ ({cart.length})</h3>
                        </div>
                        {cart.length > 0 && (
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 mb-4">
                        {cart.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50 space-y-2"><ShoppingCart size={48} /><span className="text-sm font-medium">ເລືອກສິນຄ້າເພື່ອເພີ່ມລົງໃນລາຍການ</span></div> : cart.map(item => (
                ...
                <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col min-h-[350px]">
                    <div className="flex items-center justify-between mb-3 px-2">
                        <h3 className="font-bold text-slate-700 dark:text-slate-200 text-sm">ປະຫວັດການຂໍ</h3>
                        <div className="flex items-center gap-2">
                            <div className="relative" ref={exportMenuRef}>
                                ...
                                {showExportMenu && (
                                    <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 z-50 overflow-hidden">
                                        <div className="p-1">
                                            <button onClick={() => handleExport('current')} className="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 rounded-lg">📄 Export ທີ່ເລືອກ</button>
                                            <button onClick={() => handleExport('today')} className="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 rounded-lg">📅 Export ມື້ນີ້</button>
                                            <button onClick={() => handleExport('pending')} className="w-full text-left px-3 py-2 text-xs font-bold text-orange-600 hover:bg-orange-50 rounded-lg">⏳ Export ລາຍການຄ້າງ</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl mb-4">
                        {['all', 'pending', 'accepted'].map(f => (
                            <button key={f} onClick={() => setFilter(f)} className={`flex-1 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${filter === f ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{f === 'all' ? 'ທັງໝົດ' : f === 'pending' ? 'ລໍຖ້າ' : 'ສຳເລັດ'}</button>
                        ))}
                    </div>
                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                        {groupedHistory.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-1 opacity-60"><Clock size={24} /><span className="text-xs font-medium">No history</span></div> : groupedHistory.map(batch => (
                            <div key={batch.batch_id} className="bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden text-xs">
                                <div onClick={() => setExpandedBatchId(expandedBatchId === batch.batch_id ? null : batch.batch_id)} className="p-3 flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors">
                                    <div className="flex flex-col"><span className="font-black text-slate-700 dark:text-slate-200 flex items-center gap-1.5"><span className="text-blue-500">{batch.request_by || 'Staff'}</span> <span>•</span> {new Date(batch.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span><span className="text-[10px] text-slate-400 flex items-center gap-1"><Clock size={10} /> {batch.items.length} Items</span></div>
                                    <div className="flex items-center gap-2"><span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider ${batch.status === 'accepted' ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'}`}>{batch.status}</span><ChevronDown size={14} className={`transition-transform ${expandedBatchId === batch.batch_id ? 'rotate-180' : ''}`} /></div>
                                </div>
                                {expandedBatchId === batch.batch_id && <div className="bg-white dark:bg-slate-950/30 p-2 border-t border-slate-100 dark:border-slate-800 space-y-1">{batch.items.map((item, idx) => (<div key={idx} className="flex justify-between items-center p-1.5 rounded hover:bg-slate-50"><div className="flex-1 truncate font-medium text-slate-700 dark:text-slate-300">{item.product_name} <br /><span className="text-[9px] text-slate-400">{item.barcode}</span></div><span className="font-black">x{item.qty}</span></div>))}</div>}
                            </div>
                        ))}
                    </div>
                </div>
